...
def yolo_forward(output, conf_thresh, num_classes, anchors, num_anchors, only_objectness=1,
    ...
    # bx_free = bxy[:, 6] + torch.tensor(grid_x, device=device, dtype=torch.float32)
    # by_free = bxy[:, 7] + torch.tensor(grid_y, device=device, dtype=torch.float32)
    # bw_free = bwh[:, 6] * 4
    # bh_free = bwh[:, 7] * 4
    # bx_list.append(bx_free)
    # by_list.append(by_free)
    # bw_list.append(bw_free)
    # bh_list.append(bh_free)
