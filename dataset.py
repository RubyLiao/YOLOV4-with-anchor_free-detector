...
def fill_truth_detection(bboxes, num_boxes, classes, flip_up, flip_left, dx, dy, sx, sy, net_w, net_h):
    if bboxes.shape[0] == 0:
        return bboxes, 10000
...
def image_data_augmentation(mat, w, h, pleft, ptop, swidth, sheight, flip_up, flip_left, dhue, dsat, dexp, gaussian_noise, blur,
                            truth):
    try:
        img = mat
        oh, ow, _ = img.shape
        ...
        if flip_up:
            # cv2.Mat cropped
            sized = cv2.flip(sized, 0)  # 0 - x-axis, 1 - y-axis, -1 - both axes (x & y)
...
class Yolo_dataset(Dataset):
    def __init__(self, data_txt, cfg):
        ...
        data_lines = [f.strip('\n') for f in data_lines]
        txt_lines = [line[:-3].replace("JPEGImages", "Annotations_txt") + "txt" for line in data_lines]
        for path in txt_lines:
            ...
            with open(path, 'r') as f:
                ...
                for line in lines:
                    try:
                        ...
                        cls_id, x1, y1, x2, y2, x_c, y_c = items
                        truth[name].append([x1, y1, x2, y2, cls_id])
    ...
    def __getitem__(self, index):
        img_name = list(self.truth.keys())[index]
        bboxes = np.array(self.truth.get(img_name), dtype=np.float)
        img_path = os.path.join(self.cfg.dataset_dir, img_name)
        ...
        dhue, dsat, dexp, flip_up, flip_left, blur = 0, 0, 0, 0, 0, 0
        gaussian_noise = 0
        ...
        for i in range(use_mixup + 1):
            ...
            flip_up = random.randint(0, 1)
            flip_left = random.randint(0, 1)
            ...
            truth, min_w_h = fill_truth_detection(bboxes, self.cfg.boxes, self.cfg.classes, flip_up, flip_left, pleft, ptop, swidth,
                                                  sheight, self.cfg.w, self.cfg.h)
            if (min_w_h / 8) < blur and blur > 1:  # disable blur if one of the objects is too small
                blur = min_w_h / 8

            ai = image_data_augmentation(img, self.cfg.w, self.cfg.h, pleft, ptop, swidth, sheight, flip_up, flip_left,
